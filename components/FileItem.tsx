    ...
    <div className="p-6 hover:bg-slate-50/80 transition-colors group flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto relative">
        <button
          ...
          title="Note"
        >
          <MessageSquare className="w-6 h-6 mx-auto relative" />

          {notesCount > 0 && (
            <span className="absolute top-1 left-[calc(50%+2px)] w-5 h-5 bg-rose-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white">
              {notesCount}
            </span>
          )}
        </button>
